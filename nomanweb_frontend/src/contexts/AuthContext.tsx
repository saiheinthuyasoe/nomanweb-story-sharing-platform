...
export const tokenRefreshEvents = {
  ...
  notify: (token: string, refreshToken: string) => {
    tokenRefreshEvents.listeners.forEach((callback) =>
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const register = async (userData: RegisterData) => {
    try {
      ...
      if (response.token && response.refreshToken) {
        ...
        router.push(
          "/verify-email-pending?email=" + encodeURIComponent(userData.email)
        );
      }
    } catch (error: any) {
  ...
  const setAuthData = (newToken: string, refreshToken: string, user: User) => {
  ...
  const updateTokens = (newToken: string, refreshToken: string) => {
