    ...
    try {
      ...
      for (const item of selectedItems) {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/api/admin/featured-content/${item.id}/duration?startDate=${encodeURIComponent(startDate)}${newEndDate ? `&endDate=${encodeURIComponent(newEndDate)}` : ''}`,
          {
            method: "PUT",
            headers: {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="max-h-32 overflow-y-auto border border-gray-200 rounded-md">
              {selectedItems.map((item) => (
                <div key={item.id} className="flex items-center space-x-3 p-3 border-b border-gray-100 last:border-b-0">
                  <img
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      {item.endDate ? `Expires: ${new Date(item.endDate).toLocaleDateString()}` : 'Permanent'}
                    </p>
                  </div>
            ...
            <div className="flex space-x-4">
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => setUpdateMode(e.target.value as 'set' | 'extend')}
                  className="mr-2"
                />
                <span className="text-sm text-gray-700">Set new expiration date</span>
              </label>
              <label className="flex items-center">
                <input
                  type="radio"
                  ...
                  className="mr-2"
                />
                <span className="text-sm text-gray-700">Extend current expiration</span>
              </label>
            </div>
          </div>
            ...
            <div className="flex items-center space-x-2 text-sm text-gray-600">
              ...
              <span>
                {updateMode === 'set' 
                  ...
                  : (endDate 
                      ? `Will extend by: ${Math.ceil((endDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))} days` 
        ...
        <div className="flex justify-end space-x-3 p-6 border-t border-gray-200">
          ...
          <button
            ...
            disabled={loading || (updateMode === 'set' && !endDate && selectedDuration !== 'permanent') || (updateMode === 'extend' && !endDate)}
