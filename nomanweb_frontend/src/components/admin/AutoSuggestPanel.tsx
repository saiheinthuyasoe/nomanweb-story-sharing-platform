...
const AutoSuggestPanel: React.FC<AutoSuggestPanelProps> = ({
  ...
  const loadSuggestions = async () => {
    try {
      ...
      const allSuggestions = await bookInsightsService.getSuggestedBooks(
        criteria
      );
  ...
  return (
    <Card className={className}>
      <CardHeader className="pb-3">
        ...
        <p className="text-xs text-gray-500 mt-1">{getSectionDescription()}</p>
      </CardHeader>
        ...
        {suggestions.length === 0 ? (
          ...
          suggestions.map((book, index) => (
            <div key={book.id}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span>
                            {getMetricValue(book)} {getMetricLabel()}
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              {index < suggestions.length - 1 && <Separator className="mt-3" />}
            </div>
          ))
        )}
