...
export function QuickCreateChapter({ 
  ...
  const handleQuickCreate = async () => {
    ...
    createChapter(chapterData, {
      onSuccess: (newChapter) => {
        ...
        router.push(`/stories/${storyId}/chapters/${newChapter.chapterNumber}/edit`);
      },
      onError: (error) => {
  ...
  if (variant === 'card') {
    return (
      <button
        ...
        disabled={isCreating}
        className={`group relative overflow-hidden rounded-lg border-2 border-dashed border-gray-300 hover:border-blue-500 transition-all duration-200 p-8 ${className}`}
      >
        <div className="flex flex-col items-center justify-center space-y-3">
          <div className="p-3 bg-gray-100 rounded-full group-hover:bg-blue-50 transition-colors">
            <DocumentPlusIcon className="w-8 h-8 text-gray-600 group-hover:text-blue-600" />
          </div>
  ...
  return (
    <button
      onClick={handleQuickCreate}
      disabled={isCreating}
      className={`inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
    >
      {isCreating ? (
        <>
