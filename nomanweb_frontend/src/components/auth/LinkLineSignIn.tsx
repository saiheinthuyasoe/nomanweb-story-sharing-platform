...
export default function LinkLineSignIn({ 
  onSuccess, 
  onError, 
  className = '',
  children 
  ...
  const handleLineLink = async () => {
    setIsLoading(true);
    try {
      // LINE OAuth flow for account linking - redirect to LINE authorization
      ...
      // Store state and mode for verification
      sessionStorage.setItem('line_oauth_state', state);
      sessionStorage.setItem('line_oauth_mode', 'link'); // Mark as linking mode
      localStorage.setItem('line_oauth_state', state);
      localStorage.setItem('line_oauth_mode', 'link');
      ...
      console.log('LINE OAuth Link URL:', lineAuthUrl);
      ...
      toast.error('LINE account linking failed');
      onError?.(error);
      setIsLoading(false);
    }
  };
