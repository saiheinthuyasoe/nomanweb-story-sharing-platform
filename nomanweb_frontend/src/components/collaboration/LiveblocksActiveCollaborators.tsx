...
interface LiveblocksActiveCollaboratorsProps {
  chapterId: string;
}

export const LiveblocksActiveCollaborators: React.FC<LiveblocksActiveCollaboratorsProps> = ({ chapterId }) => {
  const { user } = useAuth();
  const { collaborators, isConnected } = useLiveblocksCollaboration(chapterId);
