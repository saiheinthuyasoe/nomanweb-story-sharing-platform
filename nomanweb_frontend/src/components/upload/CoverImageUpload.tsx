...
export default function CoverImageUpload({
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (result.success && result.imageUrl) {
        console.log('🎉 Upload successful, calling onChange with:', result.imageUrl);
  ...
  const handleFileForCrop = useCallback((file: File) => {
    console.log('📁 File selected for crop:', file.name, file.size, file.type);
  ...
  const handleCroppedImage = useCallback(async (croppedFile: File) => {
    console.log('✂️ Cropped file received:', croppedFile.name, croppedFile.size, croppedFile.type);
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    console.log('📁 Files dropped:', files.length, files.map(f => ({ name: f.name, size: f.size, type: f.type })));
    if (files.length > 0) {
  ...
  const handleFileSelect = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log('📁 Files selected via input:', files?.length, files ? Array.from(files).map(f => ({ name: f.name, size: f.size, type: f.type })) : 'No files');
      ...
      <div className="text-center">
        ...
        >
          <div className="text-center relative z-10">
            ...
            <p className="text-sm font-semibold text-gray-800 group-hover:text-[#18243c] transition-colors duration-300">Add Cover Image</p>
            <p className="text-xs text-gray-500 mt-1 group-hover:text-[#22325a]/70 transition-colors duration-300">Click to upload or drag & drop</p>
          </div>
  ...
  const ModeSelector = () => (
    <div className="space-y-4">
      <div className="text-center">
        ...
        <h3 className="text-lg font-bold text-gray-900 mb-1">Add Cover Image</h3>
        <p className="text-sm text-gray-600">Choose your preferred upload method</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h4 className="font-semibold text-gray-900 text-sm mb-1 group-hover:text-[#18243c] transition-colors duration-300">Upload File</h4>
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h4 className="font-semibold text-gray-900 text-sm mb-1 group-hover:text-emerald-700 transition-colors duration-300">From URL</h4>
            <p className="text-xs text-gray-500 group-hover:text-gray-600 transition-colors duration-300">
              Web link
            </p>
          </div>
