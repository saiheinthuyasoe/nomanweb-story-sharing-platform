...
import { X, Check, RotateCcw, ZoomIn, ZoomOut, RotateCw, RotateCcw as RotateCcwIcon, Maximize2, Minus, Plus, Image as ImageIcon, Crop as CropIcon, Move } from 'lucide-react';
...
export function ImageCropModal({
  ...
  const calculateFitScale = useCallback((imgWidth: number, imgHeight: number, containerWidth: number, containerHeight: number) => {
  ...
  const calculateFullSizeScale = useCallback((imgWidth: number, imgHeight: number, containerWidth: number, containerHeight: number) => {
  ...
  function onImageLoad(e: React.SyntheticEvent<HTMLImageElement>) {
    ...
    if (containerDimensions.width && containerDimensions.height) {
      // Start with full size (scale 1) if image fits, otherwise scale down to fit
      const fullSizeScale = calculateFullSizeScale(width, height, containerDimensions.width, containerDimensions.height);
      setScale(fullSizeScale);
  ...
  const handleReset = useCallback(() => {
    if (imageDimensions.width && imageDimensions.height && containerDimensions.width && containerDimensions.height) {
      const fullSizeScale = calculateFullSizeScale(
  ...
  }, [aspectRatio, imageDimensions, containerDimensions, calculateFullSizeScale]);
  ...
  const getScaleLimits = useCallback(() => {
    ...
    return {
      min: Math.max(0.1, fullSizeScale * 0.5),
      max: Math.max(3, fullSizeScale * 3)
    };
  }, [imageDimensions, containerDimensions, calculateFullSizeScale]);

  const scaleLimits = getScaleLimits();

  const handleScaleChange = useCallback((newScale: number) => {
    const clampedScale = Math.max(scaleLimits.min, Math.min(scaleLimits.max, newScale));
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-blue-100 text-xs mt-0.5">
                  {mode === 'crop' ? 'Drag corners to crop • Switch to move mode' : 'Drag image to move • Switch to crop mode'}
                </p>
              </div>
            </div>
            <button
              ...
              <div className="mb-3 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl">
                <div className="flex items-center space-x-2">
                  ...
                  <div className="flex-1">
                    <h3 className="text-xs font-semibold text-blue-900">Instructions</h3>
              ...
              >
                <div className="min-h-full min-w-full flex items-center justify-center p-3">
                  <div 
                    ...
                    onMouseLeave={mode === 'move' ? handleMouseLeave : undefined}
                  >
                    ...
                    {/* Mode indicator */}
                    <div className={`absolute top-2 right-2 px-2 py-1 rounded text-xs font-medium transition-all duration-200 ${
                      mode === 'move' 
                        ? (isDragging ? 'bg-green-500 text-white' : 'bg-blue-500 text-white')
                        : 'bg-orange-500 text-white'
                    }`}>
                      {mode === 'move' 
                        ? (isDragging ? 'Dragging...' : 'Move Mode - Click & Drag') 
                    ...
                    <div className="absolute bottom-2 left-2 bg-black/70 text-white px-2 py-1 rounded text-xs">
                      Pos: ({imagePosition.x.toFixed(0)}, {imagePosition.y.toFixed(0)})
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-3 border border-green-200">
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  <h3 className="text-xs font-semibold text-gray-900">Rotation</h3>
                </div>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-3 border border-blue-200">
                <div className="flex items-center space-x-2 mb-1">
                  ...
                  <h3 className="text-xs font-semibold text-gray-900">Aspect Ratio</h3>
                </div>
                <p className="text-xs text-gray-600">
                  {aspectRatio === 16/9 ? '16:9' : aspectRatio === 4/3 ? '4:3' : aspectRatio === 1 ? '1:1' : `${aspectRatio.toFixed(2)}:1`}
                </p>
              </div>
            </div>
          </div>
