...
export default function PurchasedContentTab({ sortBy }: PurchasedContentTabProps) {
  ...
  const groupedPurchases = sortedPurchases.reduce((groups, purchase) => {
    ...
    if (new Date(purchase.createdAt) > new Date(groups[storyId].latestPurchaseDate)) {
  ...
  const getEffectivePurchaseType = (group: any) => {
    ...
    if (currentPricingType === 'WHOLE_BOOK' && hasChapterPurchases && !hasBookPurchase) {
      return 'book';
    }
  ...
  const getPurchaseDescription = (group: any, effectivePurchaseType: string) => {
    const hasBookPurchase = !!group.bookPurchase;
    ...
    switch (effectivePurchaseType) {
      case 'book':
        ...
        if (chapterCount > 0 && !hasBookPurchase && currentPricingType === 'WHOLE_BOOK') {
      ...
      case 'chapters':
        ...
        if (hasBookPurchase && currentPricingType === 'PAID_PER_CHAPTER') {
          const chaptersAtPurchase = group.bookPurchase?.chaptersAtPurchase || 0;
          return `${chaptersAtPurchase} Chapter${chaptersAtPurchase !== 1 ? 's' : ''} • ${chaptersAtPurchase} purchased`;
        }
        // Regular chapter purchases
        return `${chapterCount} Chapter${chapterCount !== 1 ? 's' : ''} • ${chapterCount} purchased`;
      default:
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {Object.values(groupedPurchases).map((group: any) => {
          ...
          const chapterCount = group.chapterPurchases.length;
          const purchaseDescription = getPurchaseDescription(group, effectivePurchaseType);
          
          return (
            <div key={storyId} className="bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow overflow-hidden">
              {/* Main Story Card */}
              <div className="p-4">
                <div className="flex items-start space-x-4">
                  {/* Story Cover */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      <span className={`text-sm font-medium ${
                        effectivePurchaseType === 'book' ? 'text-green-600' : 'text-purple-600'
                      }`}>
                        {effectivePurchaseType === 'book' ? 'Book Purchase' : 'Chapter Purchases'}
                      </span>
                      <span className="text-sm text-gray-500">•</span>
                      <span className="text-sm text-gray-500">{formatDate(group.latestPurchaseDate)}</span>
                    </div>
                    ...
                    <div className="flex items-center space-x-4 text-sm text-gray-500 mb-3">
                      <span>by {group.story.author?.displayName || group.story.author?.username || 'Unknown'}</span>
                      <span className="font-medium text-blue-600">{group.totalCoins} coins total</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        >
                          ...
                          {effectivePurchaseType === 'book' ? 'Read Book' : 'Continue Reading'}
                        </Link>
                      </div>
                    </div>
                  </div>
              ...
              {chapterCount > 0 && isExpanded && (
                <div className="border-t border-gray-100 bg-gray-50">
                  <div className="p-4">
                    <h4 className="text-sm font-medium text-gray-700 mb-3">Purchased Chapters:</h4>
                    <div className="space-y-2">
                      {group.chapterPurchases
                        .sort((a: any, b: any) => a.chapter.chapterNumber - b.chapter.chapterNumber)
                        .map((purchase: any) => (
                          <div key={purchase.id} className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200">
                            <div className="flex-1">
                              <div className="flex items-center space-x-2">
                                ...
                                <span className="text-sm text-gray-500">•</span>
                                <span className="text-sm text-gray-600">{purchase.chapter.title}</span>
                                <span className="text-sm text-gray-500">•</span>
                                <span className="text-sm text-blue-600 font-medium">{purchase.totalCoins} coins</span>
                              </div>
