...
export default function DebugRefreshTokenPage() {
  ...
  const checkLocalTokens = () => {
    ...
    try {
      ...
      addLog(
        `🔑 Access token expires: ${info.accessToken.expires} (${
          info.accessToken.isExpired ? "EXPIRED" : "VALID"
        })`
      );
      addLog(
        `🔄 Refresh token expires: ${info.refreshToken.expires} (${
          info.refreshToken.isExpired ? "EXPIRED" : "VALID"
        })`
      );
    } catch (error) {
  ...
  const manualRefresh = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addLog(
          `🔄 New refresh token: ${data.refreshToken.substring(0, 20)}...`
        );
      } else {
  ...
  const checkServerTokens = async () => {
    ...
    try {
      ...
      data.tokens.forEach((token: any, index: number) => {
        addLog(
          `🔑 Token ${index + 1}: ${token.token} (Valid: ${
            token.isValid
      ...
      addLog(
        `❌ Token check failed: ${error.response?.status} - ${error.message}`
      );
    }
  };
  ...
  const checkReactQueryCache = () => {
    ...
    try {
      ...
      queries.forEach((query: any, index: number) => {
        ...
        addLog(
          `📋 Query ${index + 1}: ${query.queryKey.join(
            " > "
          )} (Stale: ${isStale}, Fetching: ${isFetching})`
        );
      });
    } catch (error: any) {
              ...
              {tokenInfo && (
                <div className="space-y-2 text-sm">
                  <div className="p-3 bg-gray-100 rounded">
                    ...
                    <br />
                    Time Left:{" "}
                    {Math.floor(tokenInfo.accessToken.timeLeft / 1000)}s
                  </div>
                  ...
                  <div className="p-3 bg-gray-100 rounded">
                    ...
                    <br />
                    Time Left:{" "}
                    {Math.floor(tokenInfo.refreshToken.timeLeft / 1000)}s
                  </div>
                </div>
              )}
            </div>
