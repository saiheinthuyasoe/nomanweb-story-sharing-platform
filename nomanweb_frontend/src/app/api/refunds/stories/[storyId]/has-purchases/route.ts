...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${backendUrl}/stories/${storyId}/has-purchases`,
      {
        headers: {
    ...
    if (!response.ok) {
      throw new Error("Failed to check story purchases");
    }

    const data = await response.json();
    return NextResponse.json(data);
  } catch (error) {
    console.error("Error checking story purchases:", error);
    return NextResponse.json(
      { error: "Failed to check story purchases" },
      { status: 500 }
    );
  }
}
