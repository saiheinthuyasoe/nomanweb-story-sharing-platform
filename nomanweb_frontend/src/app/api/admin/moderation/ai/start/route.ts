...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/admin/moderation/ai/start`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.message || 'Failed to start AI moderation' },
    ...
    console.error('AI moderation start API error:', error);
    return NextResponse.json(
      { error: 'Internal server error during AI moderation start' },
      { status: 500 }
    );
  }
}