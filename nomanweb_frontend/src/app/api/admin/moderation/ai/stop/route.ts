...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/admin/moderation/ai/stop`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.message || 'Failed to stop AI moderation' },
    ...
    console.error('AI moderation stop API error:', error);
    return NextResponse.json(
      { error: 'Internal server error during AI moderation stop' },
      { status: 500 }
    );
  }
}