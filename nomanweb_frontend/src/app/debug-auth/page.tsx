...
export default function DebugAuthPage() {
  ...
  const testProfileApi = async () => {
    ...
    try {
      ...
      setApiTestResult(
        `✅ Profile API Success: ${JSON.stringify(result, null, 2)}`
      );
    } catch (error: any) {
      setApiTestResult(
        `❌ Profile API Failed: ${error.message}\n${
          error.response?.data
  ...
  const testCreateStoryApi = async () => {
    ...
    try {
      ...
      setApiTestResult(
        `✅ Create Story API Success: ${JSON.stringify(result, null, 2)}`
      );
    } catch (error: any) {
      setApiTestResult(
        `❌ Create Story API Failed: ${error.message}\n${
          error.response?.data
            ? JSON.stringify(error.response.data, null, 2)
            : ""
        }`
  ...
  const testMyStoriesApi = async () => {
    ...
    try {
      ...
      setApiTestResult(
        `✅ My Stories API Success: ${JSON.stringify(result, null, 2)}`
      );
    } catch (error: any) {
      setApiTestResult(
        `❌ My Stories API Failed: ${error.message}\n${
          error.response?.data
            ? JSON.stringify(error.response.data, null, 2)
            : ""
        }`
  ...
  const testDebugUserApi = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setApiTestResult(
          `✅ Debug User API Success: ${JSON.stringify(data, null, 2)}`
        );
      } else {
        const errorData = await response.text();
        setApiTestResult(
          `❌ Debug User API Failed: ${response.status} - ${errorData}`
        );
      }
    } catch (error: any) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="space-y-2">
              <p>
                <strong>Token:</strong> {cookies.token}
              </p>
              <p>
                <strong>Refresh Token:</strong> {cookies.refreshToken}
              </p>
              <button
