...
export default function TestJwtFlowPage() {
      ...
      if (accessToken && refreshToken) {
        ...
        try {
          ...
          if (accessPayload) {
            ...
            const now = new Date();
            const timeLeft = Math.round((accessExp.getTime() - now.getTime()) / 1000);
          ...
          if (refreshPayload) {
            ...
            const now = new Date();
            const timeLeft = Math.round((refreshExp.getTime() - now.getTime()) / 1000);
  ...
  const testRefreshToken = async () => {
    ...
    try {
      ...
      addLog(`New refresh token: ${response.refreshToken ? 'Present' : 'Missing'}`);
      ...
      addLog(`❌ Refresh failed: ${error.response?.data?.message || error.message}`);
    } finally {
      setIsTesting(false);
    }
  };
  ...
  const testLogout = async () => {
    ...
    try {
      ...
      addLog(`❌ Logout API call failed: ${error.response?.status || error.message}`);
    } finally {
      setIsTesting(false);
    }
  };
  ...
  const testAutomaticRefresh = async () => {
    ...
    try {
      ...
      addLog(`❌ Automatic refresh failed: ${error.response?.status || error.message}`);
    } finally {
      setIsTesting(false);
    }
  };
    ...
    try {
      ...
      if (currentRefreshToken) {
        try {
          ...
          addLog(`New refresh token: ${newRefreshToken ? 'Present' : 'Missing'}`);
        } catch (error: any) {
          addLog(`❌ Route change refresh failed: ${error.response?.status || error.message}`);
        }
      } else {
        addLog('❌ No refresh token available');
      }
  ...
  const testWaitAndNavigate = async () => {
    ...
    try {
      ...
      addLog(`Refresh Token after 35s: ${refreshToken ? 'Present' : 'Missing'}`);
      
    } catch (error: any) {
      addLog(`❌ API call after 35s failed: ${error.response?.status || error.message}`);
      
      // Check if tokens are still present even after error
      const accessToken = Cookies.get('token');
      const refreshToken = Cookies.get('refreshToken');
      addLog(`Access Token after error: ${accessToken ? 'Present' : 'Missing'}`);
      addLog(`Refresh Token after error: ${refreshToken ? 'Present' : 'Missing'}`);
    } finally {
      setIsTesting(false);
    }
  };
  ...
  const testTokenPersistence = async () => {
    ...
    try {
      ...
      addLog(`Initial Access Token: ${initialAccessToken ? 'Present' : 'Missing'}`);
      addLog(`Initial Refresh Token: ${initialRefreshToken ? 'Present' : 'Missing'}`);
      ...
      addLog(`After 35s Access Token: ${afterWaitAccessToken ? 'Present' : 'Missing'}`);
      addLog(`After 35s Refresh Token: ${afterWaitRefreshToken ? 'Present' : 'Missing'}`);
      ...
      addLog(`After route change Access Token: ${afterRouteAccessToken ? 'Present' : 'Missing'}`);
      addLog(`After route change Refresh Token: ${afterRouteRefreshToken ? 'Present' : 'Missing'}`);
      ...
      addLog(`After API call Access Token: ${afterApiAccessToken ? 'Present' : 'Missing'}`);
      addLog(`After API call Refresh Token: ${afterApiRefreshToken ? 'Present' : 'Missing'}`);
      ...
      addLog(`After error Access Token: ${afterErrorAccessToken ? 'Present' : 'Missing'}`);
      addLog(`After error Refresh Token: ${afterErrorRefreshToken ? 'Present' : 'Missing'}`);
    } finally {
      setIsTesting(false);
    }
  };
  ...
  const testMultipleRefreshes = async () => {
    ...
    try {
      ...
      if (firstRefreshToken) {
        ...
        addLog(`Tokens updated after first refresh: ${tokensUpdated ? 'YES ✅' : 'NO ❌'}`);
        addLog(`Refresh token rotated: ${initialRefreshToken !== afterFirstRefreshToken ? 'YES ✅' : 'NO ❌'}`);
        ...
        if (secondRefreshToken) {
          ...
          const secondTokensUpdated = afterFirstAccessToken !== afterSecondAccessToken;
          addLog(`Tokens updated after second refresh: ${secondTokensUpdated ? 'YES ✅' : 'NO ❌'}`);
          addLog(`Refresh token rotated again: ${afterFirstRefreshToken !== afterSecondRefreshToken ? 'YES ✅' : 'NO ❌'}`);
          ...
          if (thirdRefreshToken) {
            ...
            addLog(`Final Access Token: ${finalAccessToken ? 'Present' : 'Missing'}`);
            addLog(`Final Refresh Token: ${finalRefreshToken ? 'Present' : 'Missing'}`);
          } else {
      ...
      addLog(`❌ Multiple refreshes test failed: ${error.response?.status || error.message}`);
  ...
  const testConsecutiveApiCalls = async () => {
    ...
    try {
      ...
      addLog(`Tokens updated after first call: ${firstTokensUpdated ? 'YES ✅' : 'NO ❌'}`);
      ...
      const afterSecondRefreshToken = Cookies.get('refreshToken');
      const secondTokensUpdated = afterFirstAccessToken !== afterSecondAccessToken;
      addLog(`Tokens updated after second call: ${secondTokensUpdated ? 'YES ✅' : 'NO ❌'}`);
      ...
      const thirdTokensUpdated = afterSecondAccessToken !== afterThirdAccessToken;
      addLog(`Tokens updated after third call: ${thirdTokensUpdated ? 'YES ✅' : 'NO ❌'}`);
      
      // Final token state
      addLog(`Final Access Token: ${afterThirdAccessToken ? 'Present' : 'Missing'}`);
      addLog(`Final Refresh Token: ${afterThirdRefreshToken ? 'Present' : 'Missing'}`);
      
    } catch (error: any) {
      addLog(`❌ Consecutive API calls test failed: ${error.response?.status || error.message}`);
    ...
    try {
      ...
      if (initialRefreshToken) {
        ...
        if (parts.length === 3) {
          ...
          if (timeLeft > 0) {
            addLog(`⏰ Waiting ${timeLeft + 10} seconds for refresh token to expire...`);
            ...
            try {
              ...
              addLog(`✅ Refresh failed as expected: ${error.response?.status || error.message}`);
            }
          } else {
      ...
      addLog(`❌ Refresh token expiration test failed: ${error.response?.status || error.message}`);
    } finally {
      setIsTesting(false);
    }
  };
  ...
  const testInterceptorRefresh = async () => {
    ...
    try {
      ...
      addLog(`Access token updated by interceptor: ${accessTokenUpdated ? 'YES ✅' : 'NO ❌'}`);
      addLog(`Refresh token updated by interceptor: ${refreshTokenUpdated ? 'YES ✅' : 'NO ❌'}`);
      addLog(`New access token: ${afterApiAccessToken ? 'Present' : 'Missing'}`);
      addLog(`New refresh token: ${afterApiRefreshToken ? 'Present' : 'Missing'}`);
      ...
      addLog(`❌ Interceptor refresh test failed: ${error.response?.status || error.message}`);
  ...
  const testDirectApiCall = async () => {
    ...
    try {
      ...
      addLog(`After direct call Access Token: ${afterDirectAccessToken ? 'Present' : 'Missing'}`);
      addLog(`After direct call Refresh Token: ${afterDirectRefreshToken ? 'Present' : 'Missing'}`);
    ...
    <div className="container mx-auto p-6">
      ...
      {user ? (
        ...
        <div className="mb-6 p-4 bg-red-100 border border-red-400 rounded">
          ...
          <p className="text-red-700">Please log in to test the authentication flow</p>
        </div>
      )}
