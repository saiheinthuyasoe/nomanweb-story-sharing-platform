...
function transformPackageData(backendPackage: any) {
  ...
  const transformed = {
    ...
    totalCoins: backendPackage.totalCoins || (backendPackage.coinAmount || backendPackage.coins) + (backendPackage.bonusCoins || 0),
    price: backendPackage.price, // THB price
    currency: 'THB',
    description: backendPackage.description || '',
    isActive: backendPackage.isActive !== undefined ? backendPackage.isActive : true,
...
export function broadcastCoinPackageUpdate(type: string, data: any) {
  ...
  const message = JSON.stringify({ type, ...transformedData, timestamp: Date.now() });
