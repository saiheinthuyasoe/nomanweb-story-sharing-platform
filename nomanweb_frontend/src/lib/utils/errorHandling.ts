...
export const handleApiError = (error: any, defaultMessage: string) => {
  if (error.response?.status === 429) {
    ...
    toast.error(`Too many attempts. Please wait a moment before trying ${action} again.`);
  } else {
...
export const handleRateLimitError = (action: string) => {
  toast.error(`Too many ${action} attempts. Please wait a moment before trying again.`);
};
...
const getActionFromUrl = (): string => {
  ...
  if (pathname.includes('/resend-verification') || pathname.includes('/verify-email-pending')) return 'sending verification email';
