...
export const useStoryReactionStatus = (
  storyId: string,
...
export const useToggleStoryLike = () => {
  ...
  return useMutation({
    ...
    onMutate: async (storyId: string) => {
      ...
      if (previousStory) {
        ...
        queryClient.setQueryData(["story", storyId], {
          ...currentStory,
          totalLikes:
            previousReaction && (previousReaction as ReactionStatus).liked
    ...
    onError: (error: any, storyId: string, context) => {
      ...
      const errorMessage =
        error.response?.data?.error || error.message || "Failed to update like";
...
// Chapter reactions
export const useChapterReactionStatus = (
  chapterId: string,
  enabled: boolean = true
) => {
  return useQuery({
...
export const useToggleChapterLike = () => {
  ...
  return useMutation({
    mutationFn: (chapterId: string) =>
      reactionsApi.toggleChapterLike(chapterId),
