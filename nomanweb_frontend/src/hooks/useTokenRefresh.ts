...
export const useTokenRefresh = () => {
  ...
  const refreshTokens = useCallback(async (): Promise<boolean> => {
    try {
      ...
      Cookies.set('token', data.token, { expires: 7, path: '/', secure: false, sameSite: 'strict' });
      Cookies.set('refreshToken', data.refreshToken, { expires: 7, path: '/', secure: false, sameSite: 'strict' });
