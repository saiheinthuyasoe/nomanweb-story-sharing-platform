...
// My reading progress
export const useMyReadingProgress = (
  page: number = 0,
  size: number = 20,
  enabled: boolean = true
) => {
  ...
  return useMutation({
    ...
    onSuccess: (data: UpdateProgressResponse, variables) => {
      ...
      queryClient.setQueryData(
        ["reading-progress", "chapter", variables.chapterId],
        {
      ...
      queryClient.invalidateQueries({
        queryKey: ["reading-progress", "story"],
      });
      queryClient.invalidateQueries({
        queryKey: ["reading-progress", "my-progress"],
      });
      queryClient.invalidateQueries({
...
export const useClearReadingHistory = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(
        `Reading history cleared (${data.deletedCount} items removed)`
      );
    },
    onError: (error: any) => {
      toast.error(
        error.response?.data?.error || "Failed to clear reading history"
      );
    },
  });
};
...
export const useResetStoryProgress = () => {
  ...
  return useMutation({
    mutationFn: (storyId: string) => readingProgressApi.resetStoryProgress(storyId),
    onSuccess: (data, storyId) => {
      // Invalidate reading progress queries for this story
      queryClient.invalidateQueries({ queryKey: ["reading-progress", "story", storyId] });
      queryClient.invalidateQueries({ queryKey: ["reading-progress"] });
      queryClient.invalidateQueries({ queryKey: ["libraries"] });

      toast.success(
        `Reading progress reset (${data.deletedCount} chapters reset)`
      );
    },
    onError: (error: any) => {
      toast.error(
        error.response?.data?.error || "Failed to reset reading progress"
      );
    },
  });
};
...
export const useAutoUpdateProgress = (chapterId: string) => {
  ...
  const updateProgressThrottled = useCallback(
    (progressPercentage: number) => {
      // Always update if progress is 100% (chapter completion)
      if (progressPercentage >= 100) {
        if (lastUpdatedProgress !== 100 && chapterId) {
          setLastUpdatedProgress(100);
      ...
      if (
        roundedProgress > 0 &&
        roundedProgress !== lastUpdatedProgress &&
        chapterId
      ) {
