...
public interface BookPurchaseRepository extends JpaRepository<BookPurchase, UUID> {
        ...
        List<BookPurchase> findByUserAndStoryOrderByPurchasedAtDesc(@Param("user") User user,
        ...
        Page<BookPurchase> findByUserAndIsRefundedTrueOrderByRefundedAtDesc(@Param("user") User user,
                        Pageable pageable);

        // Find all refunded book purchases for stories authored by a specific user
        // (paginated)
        @Query("SELECT bp FROM BookPurchase bp WHERE bp.story.author = :author AND bp.isRefunded = true ORDER BY bp.refundedAt DESC")
        Page<BookPurchase> findByStory_AuthorAndIsRefundedTrueOrderByRefundedAtDesc(@Param("author") User author,
                        Pageable pageable);
}
