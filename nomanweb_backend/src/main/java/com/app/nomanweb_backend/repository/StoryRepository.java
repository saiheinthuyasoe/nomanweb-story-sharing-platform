...
public interface StoryRepository extends JpaRepository<Story, UUID> {
        ...
        long countByAuthorIdAndStatus(@Param("userId") UUID userId, @Param("status") String status);

        // Get total views for an author
        @Query("SELECT COALESCE(SUM(s.totalViews), 0) FROM Story s WHERE s.author.id = :userId AND s.status = 'PUBLISHED'")
        long getTotalViewsByAuthor(@Param("userId") UUID userId);

        // Get total likes for an author
        @Query("SELECT COALESCE(SUM(s.totalLikes), 0) FROM Story s WHERE s.author.id = :userId AND s.status = 'PUBLISHED'")
